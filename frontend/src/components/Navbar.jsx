...
function Navbar() {
    ...
    return (
        <nav className={`navbar flex ${isHomePage ? 'justify-between' : 'justify-center'} items-center w-full px-3 h-16 bg-white shadow-md`} ref={elementRef}>
            ...
            <img src={Logo} alt="logo" className={`w-20 h-20 cursor-pointer ${!isHomePage && 'absolute'}`} onClick={handleLogoClick} />
            {isHomePage && (
                <div className='text-xl nav-list hidden lg:block'>
            ...
            {isHomePage && (
                <input
                    ...
                    className='cursor-pointer outline-none w-20 h-[5vw] md:w-28 md:h-10 lg:w-36 border rounded-md px-2'
                />
            )}
            {isHoverToggled && (
                <div className='fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-75 z-10 transition-transform transform duration-300 ease-in-out' onClick={handleOverlayClick}>
                    <div className='absolute top-0 left-0 w-64 bg-white h-full shadow-md p-4 transform translate-x-0 transition-transform duration-300 ease-in-out'>
                        <ul className='list flex flex-col gap-4'>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/" onClick={() => setisHoverToggled(false)}>Home</Link></li>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/contact-us" onClick={() => setisHoverToggled(false)}>Contact Us</Link></li>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/affiliate-disclosure" onClick={() => setisHoverToggled(false)}>Affiliate Disclosure</Link></li>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/privacy-policy" onClick={() => setisHoverToggled(false)}>Privacy policy</Link></li>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/disclaimer" onClick={() => setisHoverToggled(false)}>Disclaimer</Link></li>
                            <li className={`relative ${navItemAfterHoverCSS}`}><Link to="/terms-and-conditions" onClick={() => setisHoverToggled(false)}>Terms and conditions</Link></li>
                        </ul>
                    </div>
                </div>
            )}
