...
function TopPosts() {
    ...
    return (
        <div className='top-offers underline-offset-4 font-normal px-8'>
            <h2 className='text-2xl mt-6 w-[30vw]'>Top Offers</h2>
                ...
                {loading ? (
                    ...
                    <ol className='list-outside list-none'>
                        {topPosts.map((post, index) => {
                            const { id, title } = post;
                            return (
                                <li key={id} className='my-3 cursor-pointer visited:text-blue-600' onClick={() => handleNavigation(id)}>
